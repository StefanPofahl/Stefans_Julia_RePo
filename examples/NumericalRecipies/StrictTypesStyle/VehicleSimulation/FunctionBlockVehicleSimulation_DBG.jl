...
const A_davis       = Float32(1.5)         # N/kN  (journal + rolling resistance)
const B_davis       = Float32(0.025)       # N/(kN·km/h)
const C_davis       = Float32(0.00045)     # N/(kN·(km/h)²)
...
mutable struct LocomotiveState
    ...
    LocomotiveState() = new(0.0f0, 0.0f0, 0.0f0, 0.0f0, 0.0f0, 0.0f0, 0.0f0, 0.0f0)
end
...
function simulate_locomotive_step!(state::LocomotiveState, drive_force_request::Float32)
    ...
    F_roll    = A_davis * weight_kN                  # Rolling resistance
    F_speed   = B_davis * weight_kN * v_kmh          # Speed-dependent term
    F_air     = C_davis * weight_kN * v_kmh^2        # Air resistance (Davis form)
    F_resist  = F_roll + F_speed + F_air
    
    # Apply limits based on mode
    if drive_force_request >= 0  # Traction
        if state.v > 0.1
            F_power_limit = P_max / state.v
        else
            F_power_limit = F_max
        end
        # --- short form:
    ...
    # Physics update
    state.F_net = state.F - F_resist
    state.a = state.F_net / rTrainWeight
    ...
    return (time= state.t, position= state.s, speed= state.v * 3.6f0, 
            acceleration= state.a, force=state.F / 1000.0f0, force_net= state.F_net / 1000.0f0,
            power= Float32(state.P / 1e6))
end
...
function simulate_locomotive(_dt::Float32=0.1f0)
    ...
    t, s, v, a, F, F_net, P = [Vector{Float32}(undef, N) for _ in 1:7]
    for i = 1:N-1
    ...
    a[end]      = a[end-1]
    F[end]      = F[end-1]
    F_net[end]  = F_net[end-1]
    P[end]      = P[end-1]
    
    return t, s, v, a, F, F_net, P
end
